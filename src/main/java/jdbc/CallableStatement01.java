...
//1.Adım: Fonksiyon Kodunu Yaz.
        String sql1="CREATE OR REPLACE FUNCTION toplamaF(x NUMERIC, y NUMERIC)\n" +
                ...
                " END\n" +
                "$$";
        // 2.Adım: Fonksiyonu calıstır.
        st.execute(sql1);

        //3.Adım: Fonksiyonu cagır
       CallableStatement cst1=con.prepareCall("{? = call toplamaF(?,?)}");

        //4.Adım:Return için registerOutParameter() methodunu, parametreler ,ç,n set() methodlarından uygun olanları kullanın
        cst1.registerOutParameter(1,Types.NUMERIC);
        cst1.setInt(2,15);
        cst1.setInt(3,25);

        //5.Adım: Calıstırmak için execute() methodunu kullan.
        cst1.execute();

        //6.Adım: Sonucu cagırmak için retun data tipine gore "get"methodlarından uygun olanı getirin
